# copied from: https://github.com/Vahe1994/AQLM/blob/main/src/modelutils.py
...
def get_model(
    model_path, cache_dir,load_quantized=None, dtype="auto", device_map=None, attn_implementation=None, trust_remote_code=False
):
    if dtype == "auto":
        dtype = (
            AutoConfig.from_pretrained(model_path, trust_remote_code=trust_remote_code, cache_dir=cache_dir).torch_dtype or "auto"
...
def get_model_head_with_norm(model):
    ...
    if model.config.model_type in (*LLAMA_LIKE, "phi3", "deepseek_v2"):
...
def save_quantized_model(model: transformers.PreTrainedModel, save_dir: str):
    ...
    for layer_index, layer in enumerate(get_layers(model)):
        ...
        def set_codes_to_int32(module):
            """
            Recursively traverse the module and convert all QuantizedWeight.codes_storage to int32.
            """
            for name, submodule in module.named_children():
                if isinstance(submodule, QuantizedWeight):
                    # Convert codes to int32 if it exists
                    if submodule.codes is not None:
                        int32_codes = submodule.codes.to(torch.int32)
                        submodule.codes = nn.Parameter(int32_codes, requires_grad=False)
                else:
                    # Recursively apply to submodules
                    set_codes_to_int32(submodule)
            return module
        layer = set_codes_to_int32(layer)
