# copied from: https://github.com/Vahe1994/AQLM/blob/main/src/datautils.py
...
def get_red_pajama(nsamples, seqlen, tokenizer, eval_mode=False):
    ...
    print("Loading red_pajama from DKYoon/SlimPajama-6B")
...
if __name__ == "__main__":
    # RedPajama 데이터셋을 미리 로드하고 저장하는 스크립트
    import argparse
    
    parser = argparse.ArgumentParser(description="Save RedPajama dataset to file")
    parser.add_argument("--nsamples", type=int, default=2048, help="Number of samples to generate")
    parser.add_argument("--seqlen", type=int, default=8192, help="Sequence length")
    parser.add_argument("--model_path", type=str, default="meta-llama/Llama-3.1-8B-Instruct", help="Path to the model for tokenizer")
    parser.add_argument("--output_path", type=str, default="red_pajama_llama3_2048_8192.pth", help="Output file path")
    ...
    # RedPajama 데이터셋 로드
    print(f"Loading RedPajama dataset with {args.nsamples} samples, seqlen={args.seqlen}")
    trainloader = get_red_pajama(args.nsamples, args.seqlen, tokenizer)
    
    # 파일로 저장
    print(f"Saving trainloader to {args.output_path}")
    torch.save(trainloader, args.output_path)
    
    print(f"Successfully saved {len(trainloader)} samples to {args.output_path}")
    print(f"Each sample shape: {trainloader[0].shape}")
