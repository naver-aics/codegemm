# copied from: https://github.com/Vahe1994/AQLM/blob/main/src/aq.py
...
from codegemm.quantization.beam_search_l2 import beam_search_optimal_codes as beam_search_minimize_weight_mse
from codegemm.quantization.beam_search_xtx import beam_search_optimal_codes as beam_search_minimize_activation_mse
from codegemm.quantization.kmeans import find_nearest_cluster, fit_faiss_kmeans, fit_kmeans, fit_kmeans_1d
from codegemm.quantization.utils import IntCodes, _dequantize_weight, ellipsis, is_signed
...
class QuantizedWeight(nn.Module):
    ...
    def __init__(
        ...
        **init_kwargs,
        ...
        self.scale_group_size = self.in_features if scale_group_size == -1 else scale_group_size
        ...
        with torch.no_grad():
            ...
            # (gunho) scale_group_size
            weight_groupwise = reference_weight.reshape(
                self.out_features // out_group_size, out_group_size, self.in_features // self.scale_group_size, self.scale_group_size
            ).swapaxes(
                1, 2
            )  # [num_out_groups, num_scale_groups, out_group_size, scale_group_size]
            ...
            # (gunho) scale_group_size
            scales = weight_groupwise.norm(dim=(2, 3), keepdim=True) + self.EPS
            # shape [num_out_groups, num_scale_groups, 1, 1]
    ...
    def estimate_nbits_per_parameter(self) -> float:
        ...
        num_scale_groups = self.in_features // self.scale_group_size
        ...
        if self.scale_nbits >= 16 or 2**self.scale_nbits >= num_in_groups:  # group-wise scales in 16 bit
            ...
            scale_store = num_out_groups * 16 * num_scale_groups
        else:
            assert False
