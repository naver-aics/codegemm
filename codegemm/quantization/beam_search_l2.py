# copied from: https://github.com/Vahe1994/AQLM/blob/main/src/beam_search_l2.py
...
from codegemm.quantization.utils import _dequantize_weight, maybe_script
...
def beam_search_optimal_codes(
    ...
    if scales is not None:
        ...
        # (gunho) for group_qunatization
        # (gunho) scale.shape = [num_output_groups, num_scale_group, 1, 1]
        n = num_input_groups // scales.shape[1]
        #scale_reshaped = scales.repeat_interleave(in_group_size * n, dim=1).view(num_output_groups, num_input_groups, 1, 1)
        scale_reshaped = scales.repeat_interleave(n, dim=1).view(num_output_groups, num_input_groups, 1, 1)
        flat_unscaled_reference = flat_unscaled_reference / scale_reshaped
