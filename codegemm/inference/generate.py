...
from codegemm_linear import CodeGEMMLinear
from aqlm import QuantizedLinear
# from AQLMLinear import AQLMLinear
...
def load_model(model_name, device, backend,  
                vec_len, group_size, num_codebooks, nbits_per_codebook, 
                random_init, checkpoint_path, 
    ...
    match backend:
        case "codegemm":
            linear_class = CodeGEMMLinear
            linear_kwargs["vec_len"] = vec_len
            linear_kwargs["group_size"] = group_size
            linear_kwargs["num_codebooks"] = num_codebooks
            linear_kwargs["nbits_per_codebook"] = nbits_per_codebook
        case "aqlm":
            linear_class = QuantizedLinear
            linear_kwargs["in_group_size"] = vec_len
            linear_kwargs["out_group_size"] = 1
            linear_kwargs["num_codebooks"] = num_codebooks
            linear_kwargs["nbits_per_codebook"] = nbits_per_codebook
        case None:
...
def main(
    ...
    model, tokenizer = load_model(model_name, device, backend,
                                  vec_len, group_size, num_codebooks, 
                                  nbits_per_codebook, 
                                  random_init, 
                                  checkpoint_path, 
                                  config_path, dtype)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vec_len', type=int, default=None, help='bitwidth')
    parser.add_argument('--group_size', type=int, default=None, help='bitwidth')
    parser.add_argument('--num_codebooks', type=int, default=None, help='bitwidth')
    parser.add_argument('--nbits_per_codebook', type=int, default=None, help='bitwidth')
    ...
    parser.add_argument('--backend', type=str, default=None, help='quantization backend to use', choices=["codegemm", "aqlm", None])
    ...
    main(
        ...
        args.device, args.model_name, args.backend, args.vec_len, args.group_size, 
        args.num_codebooks, args.nbits_per_codebook, 
